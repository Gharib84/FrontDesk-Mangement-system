<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h1 class="font-bold text-2xl text-left mb-5 ml-1 text-blue-600 sm:ml-2 xsm:text-center">Invoice List</h1>
            @if (session('success'))
            ...
            @foreach ($invoices as $invoice)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-5">
                    <div class="invoice-parent py-5 flex flex-row xs:bg-red-400 xsm:flex-col justify-between">
                        <div class="first-child h-full flex flex-col align-middle">
                            <h1 class="p-5 text-6xl text-red-600 font-bold xsm:text-center">
                                Invoice #{{ $invoiceNumber }}
                            </h1>
                            <h3 class="px-5 py-0 text-black text-lg  mt-0 mb-2 font-bold xsm:text-center">
                                {{ $invoice->room->Guest_Name }}
                            </h3>
                            <p class="py-0 px-5 text-left text-slate-600 xsm:text-center">
                                Room Number: {{ $invoice->room->room_number }}
                            </p>
                            <article class="py-0 px-5 text-slate-600 xsm:text-center">
                                <span class="font-bold text-black">Details:</span>
                                {{ $invoice->details }}

                            </article>
                        </div>
                        <div class="right-child h-full flex flex-col gap-0 justify-between align-middle xsm:mx-auto">
